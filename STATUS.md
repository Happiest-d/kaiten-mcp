...
| 5 | update-task | готово (GREEN) | specs/update-task.md | tests/tools/update-task.test.ts | src/tools/update-task.ts |
...
- Всего: 78 (фича 1: 33, фича 2: 7, фича 3: 6, фича 4: 12, фича 5: 20)
- Проходят: 78
- Падают: 0
- Фаза: 4 фичи GREEN + REVIEWED, 1 фича GREEN (update-task)
...
### 2026-02-12 — Реализация фичи 5: update-task

**Задача:** Реализовать MCP-tool для обновления существующих карточек в Kaiten.

**Изменения:**
- Создана спецификация `specs/update-task.md` — детальное описание API, сценарии, примеры
- Добавлен тип `KaitenUpdateCardRequest` в `src/kaiten/types.ts` для PATCH /cards/{id}
- Добавлен тип `UpdatedTask` в `src/kaiten/types.ts` для ответа метода
- Реализован метод `updateCard(cardId, params)` в `src/kaiten/client.ts`
- Создан tool `update-task` в `src/tools/update-task.ts`
- Интегрирован в `src/server.ts`
- Написано 20 тестов: 10 для клиента, 10 для tool (все зелёные)
- Обновлена документация: `STATUS.md`, `MANUAL.md`

**Метрики:**
- Тесты: 78 (было 58, +20)
- Фичи готовы: 5 из 17 (roadmap)
- Покрытие: все сценарии из спеки

**TDD цикл:**
- RED: написаны падающие тесты
- GREEN: реализован метод клиента и tool
- REFACTOR: код соответствует стандартам (eslint passed)

**Следующие фичи (по приоритету):**
1. **get-board-cards** 🔥 — список задач на доске (КРИТИЧНО)
2. delete-task — удаление задачи
3. add-comment — добавление комментария

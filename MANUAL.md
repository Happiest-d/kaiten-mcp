...
Обновляет поля существующей задачи (карточки) в Kaiten. Можно изменить заголовок, описание, колонку, лейн, исполнителя, участников и теги.

**ВАЖНО:**
- **Нельзя переместить карточку между досками** (board_id не поддерживается API)
- **Все параметры кроме card_id опциональны** — передавайте только те поля, которые нужно изменить
...
- column_id — переместить в другую колонку
- lane_id — переместить в другой лейн
- owner_id — переназначить исполнителя
- members — изменить список участников
- tags — изменить теги
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `card_id` | number | да | ID карточки для обновления |
| `title` | string | нет | Новый заголовок (1-500 символов) |
| `description` | string | нет | Новое описание (до 50000 символов) |
| `column_id` | number | нет | ID новой колонки (для перемещения) |
| `lane_id` | number | нет | ID нового лейна (для перемещения) |
| `owner_id` | number | нет | ID нового исполнителя |
| `members` | number[] | нет | Массив user_id участников (до 20) |
| `tags` | number[] | нет | Массив tag_id (до 20) |

**Пример 1: переименовать задачу**
```json
{
  "card_id": 12345,
  "title": "Новый заголовок задачи"
...
**Пример 2: переместить в колонку "Готово" и переназначить исполнителя**
```json
{
  "card_id": 12345,
  "column_id": 105,
...
**Пример 3: обновить описание и теги**
```json
{
  "card_id": 12345,
...
Запустит 78 тестов (без реальных HTTP-запросов — всё замокано).

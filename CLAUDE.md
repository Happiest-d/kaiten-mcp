...
## Командная работа (агенты)

При создании команды агентов **обязательно** включать следующие роли:

- **reviewer** — проверка соответствия реализации спецификациям (сценарии, маппинг полей, формат ответов, покрытие тестами)
- **tech-lead** — проверка качества кода и архитектуры (TypeScript strict, DI паттерны, консистентность, error handling, безопасность)

Ревью запускается после прохождения всех тестов, параллельно обоими ролями.

## После завершения каждой фичи

По завершении реализации фичи **обязательно** выполнить:

1. **Обновить `STATUS.md`** — статус фичи, количество тестов, фаза
2. **Обновить `MANUAL.md`** — добавить описание нового tool (параметры, примеры запросов/ответов)
3. **Создать коммит** — с описанием реализованной фичи
...
Файл `MANUAL.md` — пользовательская документация. **Обязательно обновлять** после добавления нового tool:

- Описание tool, параметры (таблица)
